...
from functions.models import FunctionEndpoint


class FunctionBaseView(LoginRequiredMixin, ProjectsListMixin):
    """Base class for Functions views"""
    ...
    def process_new_fields(self, new_fields, Function, form, form_invalid):
        for new_id, fields in new_fields.items():
            field_info = self.filter_endswith(initial_dict=fields, filter_by='_new_%s' % new_id)
            field_info['Function'] = Function

            Function_field = FunctionEndpoint()

            try:
                self.save_object(obj=Function_field, **field_info)
